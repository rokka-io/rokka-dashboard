...
const Header = ({updatePreview, previewImage = null, createStackBtnDisabled = false, isPreviewCurrent = false, showLoader = false}) => (
    ...
    <div className="flo-r">
      {previewImage && (
        <button
          type="button"
          href="#"
          onClick={(e) => { e.preventDefault(); updatePreview(previewImage) }}
      ...
      <button
        className={cx('rka-button rka-button-brand',
          { 'disabled flo-r': createStackBtnDisabled })}
        type="submit">
        { showLoader ? <div className="sk-cube-small sk-cube-white"><Spinner /></div> : 'Create stack' }
      </button>
    </div>
  </div>
)
